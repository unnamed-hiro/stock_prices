...
**約定タイミング (先読みなし)**: 当日の引け後に判断した注文は
`data/state/pending_orders.json` に保存され、**翌営業日の実行時にその日の始値で約定**します
(バックテスターと同一の現実的な挙動)。`config.yaml` の `simulation.execution: "close"` で
旧来の当日終値約定にも戻せます。
...
- このシステムは**投資判断の参考用**であり、利益を保証しません。
  **「必ず勝てる」システムは存在せず、それを謳うものは詐欺か過剰最適化です**
- 価格データは過去のものであり、将来の値動きを予測するものではありません
- 実取引機能は意図的に含めていません。誤発注を防ぐためです
- LLM戦略はAPI課金が発生します。`--limit` で銘柄数を絞ってください

### バックテストの既知のバイアス (正直な開示)

| バイアス | 状態 | 対処 |
|---|---|---|
| 先読み (同一終値約定) | ✅ 解消済み | 全経路で翌営業日始値約定 (`execution: next_open`) |
| 市場比較の欠如 | ✅ 解消済み | α(超過リターン)を全レポートに表示 |
| 過剰最適化 | ✅ 検出可能 | `scripts/run_walkforward.py` で期間分割の安定性検証 |
| **生存者バイアス** | ⚠️ **残存** | 銘柄リストは現存銘柄のみ。上場廃止銘柄(敗者)が除外されており、**バックテスト成績は実際より良く見えます**。無料データでは解消不可のため、レポートに警告を常時表示 |
| ファンダの先読み | ⚠️ 残存 | yfinanceは現在の財務値のみ。バックテストでは過去に現在値を適用 (ライブ運用では問題なし) |

### ウォークフォワード検証 (過剰最適化の検出)

```bash
python scripts/run_walkforward.py --strategy technical --windows 4
```

期間を複数の連続ウィンドウに分割し、各ウィンドウで独立にバックテストします。
**全ウィンドウで市場に勝てない戦略は、全期間合計で勝っていても「まぐれ」の疑いが強い**です。
ダッシュボードの「▶️ バックテスト実行」タブ内からも実行できます。
